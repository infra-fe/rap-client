...
class ImportRepositoryForm extends Component<Props, States> {
  ...
  render() {
    ...
    return (
      <section className="ImportRepositoryForm">
        ...
        <Translation>
          {(t) => (
            <form className="form-horizontal" onSubmit={(e) => { this.handleSubmit(e, t) }} style={{ padding: '0 20px', width: '660px' }}>
              <div className="rmodal-body">
                ...
                {(+version === 2 || +version === 3) && (
                  <div className="form-group row">
                    ...
                    <div className="col-sm-10">
                      <div>
                        <TextField id="outlined-basic" label={t('From URL')} variant="outlined" style={{ width: '600px' }} value={this.state.docUrl}
                          onChange={e => this.setState({ docUrl: e.target.value })} />
                      </div>
                      <div>
                        <TextField
                ...
                <div className="form-group row mb0">
                  ...
                  <div className="col-sm-10">
                    ...
                    >{disableSubmit ? `${t('importingMsg')}...` : `${t('submit')}`}
                    </Button>
  ...
  handleSubmit = async (e: any, t: any) => {
    ...
    if (!projectData && !docUrl) {
      this.props.enqueueSnackbar(t('Parsing, failure is not a valid JSON, please check the JSON format'), { variant: 'error' })
      return
    }
    this.setState({
      disableSubmit: true,
    ...
    if (docUrl && !projectData) {
      try {
        ...
        if (typeof result === 'object') {
          const { errMsg } = result
          if (errMsg) {
            this.props.enqueueSnackbar(errMsg, { variant: 'warning' })
            this.setState({
              disableSubmit: false,
            })
            return
    ...
    this.props.importRepository({ docUrl, orgId, version, projectData }, (res: any) => {
      ...
      if (res.isOk) {
        ...
        this.props.enqueueSnackbar(t('importFailed'), { variant: 'error' })
      }
    })
  }
}
...
export default connect(null, mapDispatchToProps)(withSnackbar(ImportRepositoryForm))
